class DeedDeck {
  ...
  createAllCards() {
    this.tiles.forEach((tile) => {
      ...
      card.style.setProperty('--top', `${rect.top + rect.height/2}px`);
      card.style.setProperty('--left', `${rect.left + rect.width/2}px`);
  ...
  showCard(tile) {
    ...
    card.style.setProperty('--top', isMobile ? "50%" : `${board.top + board.height/2}px`);
    card.style.setProperty('--left', isMobile ? "50%" : `${board.left + board.width/2}px`);
    card.style.setProperty('--rotation', `${tile.rotation}deg`);
